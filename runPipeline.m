...
if strcmp(globalState.parallelMode, 'object')
    ...
    for iUid = 1:numUid
        ...
        if ((currentQueued < globalState.maximumQueue) && ...
            ...
            newUid = iUid + globalState.maximumQueue;
            localUid = uidToProcess{newUid};
            oResults(newUid) = parfeval(p, @processObject, 2, ...
                globalState, globalFeatureConfig, inputState, ...
                globalPreprocessingConfig, globalOutputState, localUid);
            currentQueued = currentQueued + 1;
            logger('INFO', ['Queued UID ' localUid ' in position ' num2str(newUid) ' Total Queued: ' num2str(currentQueued)]);
        end
